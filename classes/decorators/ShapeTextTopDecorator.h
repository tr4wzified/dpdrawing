...
namespace DPDrawing {
	class ShapeTextTopDecorator : public ShapeTextDecorator {
		public:
		ShapeTextTopDecorator(string shapeUUID, TextureManager* tm, vector<unique_ptr<Shape>>* shapes, SDL_Renderer* renderer, Invoker* inv) : ShapeTextDecorator(shapeUUID, tm, shapes, renderer, inv) {
		}
		string getType() {
