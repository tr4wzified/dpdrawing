...
void DPDrawing::Invoker::Invoke() {
	SDL_Log("is mcmds empty? %d", mCmds.empty());
	if(!mCmds.empty()) {
		for(int i = 0; i < (int)mCmds.size(); i++) {
			...
			mCmds.at(i)->execute();
			executedCommands.push_back(std::move(mCmds.at(i)));
		}
...
void DPDrawing::Invoker::Undo(vector<unique_ptr<Shape>>* shapes, TextureManager* tm, TTF_Font* font, const int& BUTTON_WIDTH, const int BUTTON_HEIGHT, int* currentMode) {
	if(executedCommands.back() != nullptr) {
		undoneCommands.push_back(executedCommands.back());
		...
		*currentMode = 0;
		...
		SDL_Log("WARNING: The last element in executedCommands is a NULLPTR!");
	}
